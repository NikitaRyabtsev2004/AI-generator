...
function createEmptyApiModelDraft() {
    return {
        name: '',
        provider: '',
        endpoint: '',
...
function formatModelRegistrySummary(entry = {}) {
    if (entry.kind === 'api') {
        const provider = entry.api?.provider || 'API';
        const model = entry.api?.model || 'model';
        return `${provider} | ${model} | внешний endpoint`;
    }

    return `${entry.summary?.backend || 'neural'} | ${formatNumber(entry.summary?.trainedEpochs || 0)} эпох | ${formatNumber(entry.summary?.parameterCount || 0)} параметров`;
}
...
export default function TrainingTab({
                                        ...
                                        onResetModel,
                                        onExportModel,
                                        onRequestImportModel,
                                        onApiExportBlocked,
                                        isExportingModel = false,
                                        isImportingModel = false,
                                        apiExportBlocked = false,
                                    }) {
    ...
    const [modelCreationMode, setModelCreationMode] = useState('local');
    const [apiModelDraft, setApiModelDraft] = useState(() => createEmptyApiModelDraft());
    const [apiModelEditOpen, setApiModelEditOpen] = useState(false);
    const [apiModelEditTargetId, setApiModelEditTargetId] = useState('');
    const [apiModelEditDraft, setApiModelEditDraft] = useState(() => createEmptyApiModelDraft());
    ...
    const activeModelKind = snapshot.model?.kind || 'local';
    const isApiModel = activeModelKind === 'api';
    ...
    const hasManualSources = snapshot.sources.length > 0;
    const hasTrainingQueues = trainingQueues.length > 0;
    const canTrain = !isApiModel && (hasManualSources || hasQueuedTraining);
    ...
    const trainActionLabel = activeModelKind === 'api'
        ? 'Обучение недоступно'
        : (isPaused ? 'Продолжить обучение' : 'Начать обучение');
    ...
    const handleCreateLocalModel = async () => {
    ...
    const handleCreateApiModel = async () => {
        const nextSnapshot = await onCreateApiModel({
            ...apiModelDraft,
            name: apiModelDraft.name.trim(),
            provider: apiModelDraft.provider.trim(),
            endpoint: apiModelDraft.endpoint.trim(),
            model: apiModelDraft.model.trim(),
            apiKey: apiModelDraft.apiKey.trim(),
        });

        if (nextSnapshot) {
            setApiModelDraft(createEmptyApiModelDraft());
        }

        return nextSnapshot;
    };

    const openApiModelEditor = (entry) => {
        setApiModelEditTargetId(entry.id);
        setApiModelEditDraft({
            name: entry.name || '',
        ...
        setApiModelEditOpen(true);
    };

    const closeApiModelEditor = () => {
        setApiModelEditOpen(false);
        setApiModelEditTargetId('');
        setApiModelEditDraft(createEmptyApiModelDraft());
    };

    const handleUpdateApiModel = async () => {
        if (!apiModelEditTargetId) {
            return null;
        }

        const nextSnapshot = await onUpdateApiModel(apiModelEditTargetId, {
            ...apiModelEditDraft,
            name: apiModelEditDraft.name.trim(),
            provider: apiModelEditDraft.provider.trim(),
            endpoint: apiModelEditDraft.endpoint.trim(),
            model: apiModelEditDraft.model.trim(),
            apiKey: apiModelEditDraft.apiKey.trim(),
        });
    ...
    const floatingNotices = useMemo(() => {
        ...
        if (isPaused) {
            notices.push({
                id: 'training-paused',
                severity: 'success',
                message: 'Обучение на паузе. Можно продолжить с чекпоинта.'
            });
        }
        ...
        if (isApiModel) {
            notices.push({
                id: 'training-api-mode',
                severity: 'info',
                message: 'Активна API-модель: для чата будет использоваться внешний endpoint, локальное обучение отключено.'
            });
        }
        if (!isApiModel && modelConstraintReport.adjustments.length) {
            notices.push({
                id: 'training-adjustments',
                severity: 'info',
                message: 'Параметры авто-скорректированы под ограничения модели и памяти.'
            });
        }
        if (!isApiModel && modelConstraintReport.diagnostics.highOomRisk) {
            notices.push({
                id: 'training-oom-risk',
                severity: 'warning',
                message: `Высокий риск OOM: attention ~${formatBytes(modelConstraintReport.diagnostics.attentionTensorBytes)}, FFN ~${formatBytes(modelConstraintReport.diagnostics.ffnTensorBytes)}.`
            });
        }
        if (!canTrain && !isApiModel) {
            notices.push({
                id: 'training-no-data',
                severity: 'info',
                message: 'Добавьте хотя бы один источник (TXT/CSV/JSON/JSONL/PARQUET или URL).'
            });
        }
        if (hasQueuedTraining) {
            notices.push({
                id: 'training-queue-ready',
                severity: 'info',
                message: `В очереди обучения: ${pendingQueueItems.length}.`
            });
        }
        ...
        if (snapshot.model.computeBackendWarning) {
            notices.push({
                id: 'training-backend-warning',
                severity: 'warning',
                message: snapshot.model.computeBackendWarning
            });
        }
        ...
        <div className="training-tab">
            <div className="training-grid">
                <GlassPanel
                    ...
                    className="panel--fit panel--settings"
                >
                    <div className="panel-heading panel-heading--full">
                        <div className="panel-heading__copy">
                            <Typography variant="h3">Управление моделью и обучением</Typography>
                        </div>
                    </div>
                    ...
                    {isPaused ? (
                        <Alert severity="success">
                            Обучение остановлено на паузе. Можно продолжить с сохраненного чекпоинта.
                        </Alert>
                    ) : null}
                    {trainingLocked ? (
                        <Alert severity="info">
                            Во время обучения некоторые действия и настройки временно заблокированы.
                        </Alert>
                    ) : null}
                    {hasUnsavedSettings ? (
                        <Alert severity="warning">
                            Есть несохранённые изменения настроек.
                        </Alert>
                    ) : null}
                    {!isApiModel && modelConstraintReport.adjustments.length ? (
                        <Alert severity="info">
                            {`Автокоррекция параметров: ${modelConstraintReport.adjustments.join(' ')}`}
                        </Alert>
                    ) : null}
                    {!isApiModel && modelConstraintReport.diagnostics.highOomRisk ? (
                        <Alert severity="warning">
                    ...
                    {!canTrain ? (
                        <Alert severity="info">
                            {isApiModel
                                ? 'Для API-модели обучение отключено. Используйте ее для чата и отладки, а для обучения переключитесь на локальную модель.'
                    ...
                    {hasQueuedTraining && snapshot.sources.length > 0 ? (
                        <Alert severity="warning">
                            Ручная очередь источников и автоочереди разделены. При запуске автоочереди текущие
                            загруженные файлы из ручного списка не будут подмешаны в нее автоматически.
                        </Alert>
                    ) : null}
                    {queueRunnerAlertVisible ? (
                    ...
                    <div className="settings-scroll">
                        <div className="settings-group">
                            ...
                            <Typography variant="body2" className="muted-text">
                                Можно создать локальную модель или API-модель. Локальная модель подойдет для обучения, а
                                API-модель подключает внешний сервис через любой OpenAI-compatible endpoint.
                            </Typography>

                            <div className="toggle-row">
                                <div className="toggle-control">
                                    <Typography variant="subtitle1">Тип новой модели</Typography>
                                    <RadioGroup
                                        row
                                        value={modelCreationMode}
                                        onChange={(event) => setModelCreationMode(event.target.value)}
                                    >
                                        <FormControlLabel value="local" control={<Radio/>} label="Локальная"/>
                                        <FormControlLabel value="api" control={<Radio/>} label="API"/>
                                    </RadioGroup>
                                </div>
                            </div>

                            {modelCreationMode === 'local' ? (
                                <div className="queue-builder">
                                    ...
                                    <Button
                                        variant="outlined"
                                        className="action-button"
                                        onClick={handleCreateLocalModel}
                                ...
                                <div className="model-api-builder">
                                    <div className="queue-builder">
                                        <TextField
                                            className="text-input"
                                            label="Название API-модели"
                                            value={apiModelDraft.name}
                                            autoComplete="new-password"
                                            name="api-model-create-name"
                                            inputProps={{autoCapitalize: 'none', spellCheck: 'false'}}
                                            disabled={trainingLocked}
                                            onChange={(event) => setApiModelDraft((current) => ({
                                                ...current,
                                                name: event.target.value
                                            }))}
                                            placeholder="Например, OpenRouter debug"
                                        />
                                        <TextField
                                            className="text-input"
                                            label="Провайдер"
                                            value={apiModelDraft.provider}
                                            autoComplete="new-password"
                                            name="api-model-create-provider"
                                            inputProps={{autoCapitalize: 'none', spellCheck: 'false'}}
                                            disabled={trainingLocked}
                                            onChange={(event) => setApiModelDraft((current) => ({
                                                ...current,
                                            ...
                                            placeholder="OpenRouter, Groq, Hugging Face"
                                        />
                                    </div>
                                    <div className="queue-builder">
                                        <TextField
                                            className="text-input"
                                            label="Endpoint"
                                            value={apiModelDraft.endpoint}
                                            autoComplete="new-password"
                                            name="api-model-create-endpoint"
                                            inputProps={{autoCapitalize: 'none', spellCheck: 'false'}}
                                            disabled={trainingLocked}
                                            onChange={(event) => setApiModelDraft((current) => ({
                                                ...current,
                                        ...
                                        <TextField
                                            className="text-input"
                                            label="Model ID"
                                            value={apiModelDraft.model}
                                            autoComplete="new-password"
                                            name="api-model-create-model"
                                            inputProps={{autoCapitalize: 'none', spellCheck: 'false'}}
                                            disabled={trainingLocked}
                                            onChange={(event) => setApiModelDraft((current) => ({
                                                ...current,
                                            ...
                                            placeholder="openai/gpt-4o-mini или meta-llama/..."
                                        />
                                    </div>
                                    <div className="queue-builder">
                                        <TextField
                                            ...
                                            label="API key"
                                            type="password"
                                            value={apiModelDraft.apiKey}
                                            autoComplete="new-password"
                                            name="api-model-create-key"
                                            inputProps={{autoCapitalize: 'none', spellCheck: 'false'}}
                                            disabled={trainingLocked}
                                            onChange={(event) => setApiModelDraft((current) => ({
                                                ...current,
                                        ...
                                        <Button
                                            variant="outlined"
                                            className="action-button"
                                            onClick={handleCreateApiModel}
                                            disabled={busy || trainingLocked || !apiModelDraft.endpoint.trim() || !apiModelDraft.model.trim() || !apiModelDraft.apiKey.trim()}
                                        >
                                            Подключить и переключить
                                        </Button>
                                    </div>
                                </div>
                            )}
                                ...
                                <div className="toggle-control">
                                    ...
                                    <div style={{display: 'flex', justifyContent: 'space-between', alignItems: 'center'}}>
                                        <Typography variant="caption" className="muted-text">
                                            По текущим настройкам видно, какой приблизительно будет размер модели.
                                            Оценка нужна для контроля архитектуры.
                                        </Typography>
                            ...
                            <div className="model-library-list">
                                {modelRegistry.length ? modelRegistry.map((entry) => (
                                    ...
                                    >
                                        <div className="model-library-card__head">
                                            <div>
                                                ...
                                                <Typography variant="caption" className="muted-text">
                                                    {formatModelRegistrySummary(entry)}
                                                </Typography>
                                            </div>
                                            <div className="training-queue-card__actions">
                                                ...
                                                {entry.kind === 'api' ? (
                                                    <Button
                                                        ...
                                                        onClick={() => openApiModelEditor(entry)}
                                                        disabled={busy || trainingLocked}
                                                    >
                                                        Редактировать
                                                    </Button>
                                    ...
                                    <Typography variant="body2" className="muted-text">
                                        В библиотеке пока нет сохранённых моделей.
                                    </Typography>
                                )}
                            </div>
                        </div>
                        ...
                        <div className="settings-group">
                            ...
                            <Typography variant="body2" className="muted-text">
                                Для генерации ответов доступен локальный Transformer или подключенный OpenAI-compatible
                                API. Здесь настраивается веб-поиск, системный промпт и общие параметры генератора.
                            </Typography>
                            ...
                            <div className="toggle-row">
                                <div className="toggle-control">
                                    ...
                                    >
                                        <FormControlLabel value="off" control={<Radio/>} disabled={trainingLocked}
                                                          label="Выключен"/>
                                        <FormControlLabel value="on" control={<Radio/>} disabled={trainingLocked}
                                                          label="Включен"/>
                                    </RadioGroup>
                                </div>
                            </div>
                            ...
                            <div className="settings-control-list">
                                <div className="settings-control">
                                    <div className="settings-control__header">
                                        <Typography variant="subtitle2">Предпочтительные домены для
                                            веб-поиска</Typography>
                                    </div>
                                    <TextField
                                        className="text-input"
                                                ...
                                                {control.options.map((option) => (
                                                    <FormControlLabel
                                                        ...
                                                        label={(
                                                            <div>
                                                                <Typography
                                                                    variant="subtitle2">{option.label}</Typography>
                                                                <Typography variant="caption" className="muted-text">
                                                                    {option.hint}
                                                                </Typography>
                                                            </div>
                            ...
                            <div className="settings-groups">
                                            ...
                                            {group.controls.map((control) => {
                                                ...
                                                const sliderMarks = hasDynamicRange
                                                    ? [
                                                        {
                                                            value: control.min,
                                                            label: `${formatRangeValue(control, control.min)}`
                                                        },
                                                        {
                                                            value: dynamicMax,
                                                            label: `лимит ${formatRangeValue(control, dynamicMax)}`
                                                        },
                                                    ]
                                                    : [
                                                        {
                                                        ...
                                                        {
                                                            value: control.max,
                                                            label: `${formatRangeValue(control, control.max)}`
                                                        },
                                                    ];
                                                    ...
                                                    <div className="settings-control" key={control.key}>
                                                        <div className="settings-control__header">
                                                            <div className="settings-control__label-wrap">
                                                                <Typography
                                                                    variant="subtitle2">{control.label}</Typography>
                              ...
                              <span className="settings-control__value">
                                {formatSettingValue(control, value)}
                                                        ...
                                                        />
                                                        {hasDynamicRange ? (
                                                            <Typography style={{marginLeft: '30px'}} variant="caption"
                                                                        className="muted-text settings-control__range-hint">
                ...
                {!isApiModel ? (
                    <div className="main-panel--plain">
                        <div className="panel-heading panel-heading--full">
                            <div className="panel-heading__copy">
                        ...
                        <div className="upload-actions">
                            ...
                            {isUploadingFiles ? (
                                <div className="upload-progress-wrap">
                                    <Typography variant="caption" className="muted-text">
                                        {isServerProcessing
                                            ? `Обработка файла на сервере: ${uploadIndicatorPercent.toFixed(2)}%`
                                            : normalizedUploadProgress < 100
                                                ? `Загрузка файла: ${normalizedUploadProgress.toFixed(2)}%`
                                                : 'Файл загружен, сервер подсчитывает токены...'}
                                    </Typography>
                                    <LinearProgress
                                        variant="determinate"
                        ...
                        <div className={`source-list ${hasManualSources ? 'source-list--active' : ''}`.trim()}>
                                ...
                                >
                                    <div className="source-card">
                                        <div className="source-card__header">
                                            ...
                                            <IconButton onClick={() => onRemoveSource(source.id)}
                                                        disabled={busy || trainingLocked}>
                                                <DeleteOutlineRoundedIcon/>
                                            </IconButton>
                                        </div>
                        ...
                        <div className="training-queues-section">
                                    ...
                                    <Typography variant="caption" className="muted-text">
                                        {isServerProcessing
                                            ...
                                            : normalizedUploadProgress < 100
                                                ...
                                                : 'Файлы очереди загружены, выполняется подготовка...'}
                                    </Typography>
                                    <LinearProgress
                                        variant="determinate"
                                        value={uploadIndicatorPercent}
                            ...
                            <div
                                className={`training-queue-list ${hasTrainingQueues ? 'training-queue-list--active' : ''}`.trim()}>
                                {trainingQueues.length ? trainingQueues.map((queue, index) => (
                                    <GlassPanel
                                        ...
                                        filterStyle={{ borderRadius: '24px'}}
                                    >
                                        <div className="training-queue-card">
                                            <div className="training-queue-card__head">
                                                <div>
                                                    <Typography
                                                        variant="subtitle2">{`${index + 1}. ${queue.name}`}</Typography>
                                                    <Typography variant="caption" className="muted-text">
                                            ...
                                            {queue.lastError ? (
                                                <Typography variant="caption"
                                                            className="muted-text training-queue-card__error">
                                                    {queue.lastError}
                                                </Typography>
                                            ) : null}
                ...
                <GlassPanel
                    style={{overflow: 'visible'}}
                    filterStyle={{borderRadius: '24px'}}
                    className="panel--fit panel--state"
                >
                    <div className="panel-heading panel-heading--full">
                        <div className="panel-heading__copy">
                ...
                <span className="training-live-alert__line">
                  ...
                  {`ETA ${trainingEtaLabel} | Окончание ${trainingEtaAtLabel} | Скорость ${trainingThroughputBps ? formatDecimal(trainingThroughputBps, 3) : '—'} батч/с | Шаг ~${trainingAvgBatchMs ? formatDecimal(trainingAvgBatchMs, 1) : '—'} мс`}
                        ...
                        <div className="metric-grid">
                            <MetricCard label="Параметры" value={formatNumber(snapshot.model.parameterCount)}
                                        hint="в модели"/>
                            <MetricCard label="Словарь" value={formatNumber(snapshot.model.vocabularySize)}
                                        hint="токенов"/>
                            <MetricCard label="Эпохи" value={formatNumber(snapshot.model.trainedEpochs)}
                                        hint="завершено"/>
                            <MetricCard label="Loss" value={formatDecimal(snapshot.model.lastLoss, 4)}
                                        hint="последний батч"/>
                            <MetricCard label="Val loss" value={formatDecimal(snapshot.model.validationLoss, 4)}
                                        hint="последняя проверка"/>
                            <MetricCard label="Оценки" value={formatNumber(snapshot.runtime?.ratedMessages || 0)}
                                        hint="ответов"/>
                        </div>
                            ...
                            <div className="server-log-list">
                                {serverLogs.length ? (
                                    serverLogs.map((entry) => (
                                        <div className={`server-log-entry server-log-entry--${entry.level || 'info'}`}
                                             key={entry.id}>
            ...
            <div className="training-bottom-grid">
                <GlassPanel
                    style={{overflow: 'visible'}}
                    filterStyle={{borderRadius: '24px'}}
                    className="panel--fit panel--chart"
                >
                    <div className="panel-heading panel-heading--full">
                        <div className="panel-heading__copy">
            ...
            {apiModelEditOpen ? (
                <div
                    className="training-api-modal"
                    role="dialog"
                    aria-modal="true"
                    aria-label="Редактирование API-модели"
                    onMouseDown={(event) => {
                ...
                >
                    <div className="training-api-modal__backdrop" onMouseDown={closeApiModelEditor}/>
                    <div className="training-api-modal__panel">
                <DialogTitle>Редактирование API-модели</DialogTitle>
                <DialogContent className="training-api-dialog__content">
                    <Typography variant="body2" className="muted-text">
                        Если API key нужно оставить прежним, можно сохранить форму пустой.
                    </Typography>
                    <TextField
                        className="text-input"
                        label="Название API-модели"
                        value={apiModelEditDraft.name}
                        autoComplete="new-password"
                        name="api-model-edit-name"
                        inputProps={{autoCapitalize: 'none', spellCheck: 'false'}}
                        onChange={(event) => setApiModelEditDraft((current) => ({
                            ...current,
                            name: event.target.value
                        }))}
                    />
                    <TextField
                        className="text-input"
                        label="Провайдер"
                        value={apiModelEditDraft.provider}
                        autoComplete="new-password"
                        name="api-model-edit-provider"
                        inputProps={{autoCapitalize: 'none', spellCheck: 'false'}}
                        onChange={(event) => setApiModelEditDraft((current) => ({
                            ...current,
                            provider: event.target.value
                    ...
                    <TextField
                        className="text-input"
                        label="Endpoint"
                        value={apiModelEditDraft.endpoint}
                        autoComplete="new-password"
                        name="api-model-edit-endpoint"
                        inputProps={{autoCapitalize: 'none', spellCheck: 'false'}}
                        onChange={(event) => setApiModelEditDraft((current) => ({
                            ...current,
                            endpoint: event.target.value
                    ...
                    <TextField
                        className="text-input"
                        label="Model ID"
                        value={apiModelEditDraft.model}
                        autoComplete="new-password"
                        name="api-model-edit-model"
                        inputProps={{autoCapitalize: 'none', spellCheck: 'false'}}
                        onChange={(event) => setApiModelEditDraft((current) => ({
                            ...current,
                            model: event.target.value
                    ...
                    <TextField
                        ...
                        label="Новый API key"
                        type="password"
                        value={apiModelEditDraft.apiKey}
                        autoComplete="new-password"
                        name="api-model-edit-key"
                        inputProps={{autoCapitalize: 'none', spellCheck: 'false'}}
                        onChange={(event) => setApiModelEditDraft((current) => ({
                            ...current,
                            apiKey: event.target.value
                ...
                <DialogActions>
                    <Button variant="text" onClick={closeApiModelEditor} disabled={busy || trainingLocked}>
                        Отмена
                    </Button>
                    <Button
                        variant="contained"
                        onClick={handleUpdateApiModel}
                        color='inherit'
                        disabled={
                            busy ||
                            trainingLocked ||
                            !apiModelEditDraft.endpoint.trim() ||
                            !apiModelEditDraft.model.trim()
                        }
                    >
                        Сохранить
                    </Button>
